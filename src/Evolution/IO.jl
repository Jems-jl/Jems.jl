...
Returns the value of the independent variable `var_symbol` at either the surface or
the center of the StellarModel `sm`. `edge` can be either `:surface` or `:center`.
"""
...
history_output_options = Dict(
                              ...
                              "X_surf" => ("unitless",
                                           sm -> history_get_ind_vars_edge_value(sm, :H1,
                                                                                 :surface)),
                              "Y_surf" => ("unitless",
                                           sm -> history_get_ind_vars_edge_value(sm, :He4,
                                                                                 :surface)),
...
Returns the value of the variable Symbol `var_symbol` at cell number `k` of the StellarModel
`sm`.
"""
...
profile_output_options = Dict(
                              ...
                              "log10_r" => ("log10(Rsun)",
                                            (sm, k) -> sm.esi.lnr[k] / RSUN / log(10)),
                              "log10_P" => ("log10(dyne)",
                                            (sm, k) -> sm.esi.lnP[k] / log(10)),
                              "log10_T" => ("log10(K)", (sm, k) -> sm.esi.lnT[k] / log(10)),
                              "log10_ρ" => ("log10_(g*cm^-3)",
                                            (sm, k) -> log10(get_eos_for_cell(sm, k)[1])),
                              "luminosity" => ("Lsun", (sm, k) -> sm.esi.L[k]),

                              #abundance
                              "X" => ("unitless",
                                      (sm, k) -> profile_get_ind_vars_value(sm, :H1, k)),
                              "Y" => ("unitless",
                                      (sm, k) -> profile_get_ind_vars_value(sm, :He4, k)))
    ...
    if (sm.opt.io.history_interval > 0)
        ...
        if !file_exists  # create file if it doesn't exist yet
            h5open(sm.opt.io.hdf5_history_filename, "w") do history_file
                ...
                # Dataset is created with size (0, ncols), we will add rows by using
                # the HDF5.set_extent_dims function the (-1, ncols) is used to define
                # the maximum extent of the dataset, -1 indicates that it is unbound
                # in number of rows. The chunk size is used for compression. Smaller
                # chunk sizes will result in worse compression but faster writes.
                # The compression level can be anywhere between 0 and 9, 0 being no
                # compression 9 being the highest. Compression is lossless.
                history = create_dataset(history_file,
                                         "history",
                                         Float64,
                                         ((0, ncols), (-1, ncols));
                ...
                attrs(history)["column_units"] = [history_output_options[data_cols[i]][1]
                                                  for i in eachindex(data_cols)]
                # Finally, place column names
                attrs(history)["column_names"] = [data_cols[i]
                                                  for i in eachindex(data_cols)]
            end
        end
        ...
        if (sm.model_number % sm.opt.io.profile_interval == 0)
            h5open(sm.opt.io.hdf5_profile_filename, "r+") do profile_file
                ...
                profile = create_dataset(profile_file,
                                         ...
                                         Float64,
                                         ((sm.nz, ncols), (sm.nz, ncols));
                                         chunk=(sm.opt.io.hdf5_profile_chunk_size, ncols),
                                         compress=sm.opt.io.hdf5_profile_compression_level)

                # next up, include the units for all quantities. No need to recheck columns.
                attrs(profile)["column_units"] = [profile_output_options[data_cols[i]][1]
                                                  for i in eachindex(data_cols)]
                # Place column names
                attrs(profile)["column_names"] = [data_cols[i]
                                                  for i in eachindex(data_cols)]
...
function get_history_dataframe_from_hdf5(hdf5_filename)
    h5open(hdf5_filename) do history_file
        return DataFrame(history_file["history"][:, :],
                         attrs(history_file["history"])["column_names"])
    end
end
...
function get_profile_dataframe_from_hdf5(hdf5_filename, profile_name)
    h5open(hdf5_filename) do profile_file
        return DataFrame(profile_file[profile_name][:, :],
                         attrs(profile_file[profile_name])["column_names"])
    end
end
