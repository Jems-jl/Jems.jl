...
# - `κ00`: Opacity evaluated at the current cell (`k`)  # atmosphere boundary condition

# - `κp1`: Opacity evaluated at the next cell (`k+1`)

# Returns
  # Eddington gray, ignoring radiation pressure term
...
function equationJrot(sm::StellarModel, k::Int)
    ...
    irot₀ = get_00_dual(sm.props.i_rot[k])
    ω₀ = get_00_dual(sm.props.ω[k])
    ν₀ = get_00_dual(sm.props.ν_ω[k])

    F₊::typeof(jrot₀) = 0
    F₋::typeof(jrot₀) = 0
    if k < sm.props.nz
        F₊ = pi^2 * ρface₊^2 * r₀^4 * (irot₀ + get_p1_dual(sm.props.i_rot[k + 1])) *
             (ν₀ + get_p1_dual(sm.props.ν_ω[k + 1])) * (get_p1_dual(sm.props.ω[k+1]) - ω₀) / sm.props.dm[k]
    end
    if k > 1
        F₋ = pi^2 * (exp(get_m1_dual(sm.props.lnρ_face[k-1])))^2 * (exp(get_m1_dual(sm.props.lnr[k-1])))^4 * 
