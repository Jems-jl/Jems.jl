...
"""
struct FaceDualData{TWONVARSP1, THREENVARSP1, TNUMBER, TAG}
...
"""
function FaceDualData(nvars::Int, ::Type{TNUMBER}, ::Type{DUAL_TAG}) where {TNUMBER, DUAL_TAG}
    diff_cache_face = StarDiffCache(2*nvars, TNUMBER, DUAL_TAG)
    diff_cache_m1 = StarDiffCache(3*nvars, TNUMBER, DUAL_TAG)
    diff_cache_00 = StarDiffCache(3*nvars, TNUMBER, DUAL_TAG)
    fd = FaceDualData{2*nvars+1, 3*nvars+1, TNUMBER, DUAL_TAG}(diff_cache_face, 
                                diff_cache_00, diff_cache_m1)
    return fd
end

function Base.zero(::Type{FaceDualData{SIZE1,SIZE2,TNUMBER,DUAL_TAG}}) where {SIZE1, SIZE2, TNUMBER, DUAL_TAG}
    return FaceDualData((SIZE1-1)÷2, TNUMBER, DUAL_TAG)
end

function Base.convert(::Type{FaceDualData{SIZE1, SIZE2, TN1, DUAL_TAG}}, x::TN2) where {SIZE1, SIZE2, TN1<:Number, TN2<:Number, DUAL_TAG} 
    cd = zero(FaceDualData{SIZE1,SIZE2,TN1,DUAL_TAG})
...
function update_face_dual_data!(fd::FaceDualData{SIZE1, SIZE2, TNUMBER, DUAL_TAG}, dual::TDSC) where {SIZE1, SIZE2, TNUMBER, TDSC, DUAL_TAG}
