...
function get_μ_IdealEOS(isotope_data::Dict{Symbol, Isotope}, xa::Vector{<:TT}, species::Vector{Symbol})::TT where{TT<:Real}
    ...
    for i in eachindex(species)
        μ += xa[i]*(1+isotope_data[species[i]].Z)/isotope_data[species[i]].mass  # assumes complete ionization!
    end
    return 1/μ
end
...
computes thermodynamical quantities of a mixture `xa` at temperature `lnT` and pressure `lnP`, given
the ideal equation of state `eos`, `isotope_data` and list of `species`.
"""
