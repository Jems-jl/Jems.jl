...
folder_path = "/Users/evakuipers/Downloads/BM_MM_SA_output"
...
y_err_top = m_list .- q10_list 
y_err_bot = q90_list .- m_list
...
function least_squares_fit(xdata, ydata)
    ...
    X = hcat(ones(n), xdata) 
    β = X \ ydata
    cov_matrix = inv(X' * X)  # Covariance matrix without weights
    return β, cov_matrix
end
...
println("*** Values for MM - using StaticArrays ***")
println("Intercept A: ", α[1])
println("Slope A: ", α[2])
