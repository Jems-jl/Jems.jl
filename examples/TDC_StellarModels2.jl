...
import Jems.DualSupport: get_00_dual
using ForwardDiff
...
@kwdef mutable struct TDCStellarModelProperties{TN, TDual, TDualMixed, TLocalDualData, TMixedDualData} <: Jems.StellarModels.AbstractModelProperties
    ...
    turb_flux_cc::Vector{TMixedDualData}   # erg K^-1 g^-1
    #D_turb 
...
function TDCStellarModelProperties(nvars::Int, nz::Int, nextra::Int, nrates::Int, nspecies::Int, vari::Dict{Symbol,Int},
    ...
    turb_flux_cc = Vector{MDDTYPE}(undef, nz+nextra)#zeros(LDDTYPE, nz+nextra)
    ...
    for k in 1:(nz+nextra)
        ...
        turb_flux_cc[k] = MixedDualData(nvars, TN)
...
function Jems.StellarModels.evaluate_stellar_model_properties!(sm, props::TDCStellarModelProperties)
    ...
    gamma_00_dual   = abs(get_mixed_00_dual(props.gamma_turb[i]))
    gamma_p1_dual = abs(get_mixed_p1_dual(props.gamma_turb[i+1]))
    gamma_turb_dual = (props.dm[i+1] * gamma_00_dual + props.dm[i] * gamma_p1_dual) / (props.dm[i] + props.dm[i+1])
    update_mixed_dual_data!(props.gamma_turb_face[i], gamma_turb_dual)
        ...
        # m_solar = props.m[i] / MSUN
        # if m_solar < 4.8
        #     D_blended = D_turb_dual
        # elseif m_solar > 4.9
        #     D_blended = props.turb_res_dual[i].D_turb
        # else
        #     w = smooth_step_func(m_solar, 4.8, 4.9)
        #     D_blended = (1.0 - w) * D_turb_dual + w * props.turb_res_dual[i].D_turb
        # end

        # flux_term_dual = (4π * r_dual^2 * ρ_face_dual)^2 * D_blended / 0.5 * (props.dm[i] + props.dm[i+1])
        # update_mixed_dual_data!(props.flux_term[i], flux_term_dual)
    ...
    Threads.@threads for i in 1:sm.props.nz-1
        ## P1 values ##
        if i != props.nz-1
            P_cc_p1 = get_mixed_p1_dual(props.eos_res[i+1].P)
            ρ_cc_p1 = get_mixed_p1_dual(props.eos_res[i+1].ρ)
        else
            P_face_p1 = get_mixed_p1_dual(props.eos_res[i+1].P)
            ρ_face_p1 = get_mixed_p1_dual(props.eos_res[i+1].ρ)
            P_cc_00 = get_mixed_00_dual(props.eos_res[i].P)
            ρ_cc_00 = get_mixed_00_dual(props.eos_res[i].ρ)
        ...
        r_cc_p1 = 0.5*(exp(get_mixed_p1_dual(props.lnr[i+1])) + exp(get_mixed_00_dual(props.lnr[i])))

        Hₚ_cc_p1 = P_cc_p1 / (ρ_cc_p1 * m_face_p1 * CGRAV / r_cc_p1^2)
        Λ_cc_p1 = 1 / (1 / Hₚ_cc_p1 + 1 / r_cc_p1)
        
        # CHANGED: exp -> sin
        γ_cc_p1 = 0.5 * (abs(sinh(get_mixed_p1_dual(sm.props.gamma_turb[i+1]))) + abs(sinh(get_mixed_00_dual(sm.props.gamma_turb[i]))))
        A_p1 = (4π *ρ_cc_p1* r_cc_p1^2)^2 * Λ_cc_p1 * α_w_FACTOR[] * ( γ_cc_p1)  #sqrt(ω_cc_p1)    
        
        # CHANGED: exp -> sin
        turb_flux_term = (A_p1 / props.dm[i+1]) * ((sinh(get_mixed_p1_dual(props.gamma_turb[i+1])))^2 - (sinh(get_mixed_00_dual(props.gamma_turb[i])))^2)
        update_mixed_dual_data!(sm.props.turb_flux_cc[i], turb_flux_term)
    end
end
...
function gammaTurb_blend(sm::StellarModel, k::Int)
    ...
    γ_target = asinh(abs((v_turb_floored) / sqrt(2.0)))
