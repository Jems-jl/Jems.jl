...
for (i,logmass) in enumerate(logmassrange)
    ...
    for (j,X) in enumerate(Xrange)
        ...
        n=3
        #StellarModels.n_polytrope_initial_condition!(n, sm, X, Z,Dfraction ,Chem.abundance_lists[:ASG_09],mass*MSUN, 100 * RSUN; initial_dt=10 * SECYEAR)
